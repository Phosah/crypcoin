<footer class="py-20 border-t border-gray-300">
    <div class="flex items-start justify-center mx-auto max-w-4xl px-8">
        <div class="flex-1">
            <img class="w-32" src="img/footer-logo.png" alt="Crypcoin Logo">
        </div>
        <div class="flex-1">
            <h3 class="text-brand-gray-dark-1 font-semibold text-sm mb-4">Quick Links</h3>
            <p class="text-sm text-brand-gray-light">FAQ</p>
        </div>
        <div class="flex-1">
            <h3 class="text-brand-gray-dark-1 font-semibold text-sm mb-4">Legal</h3>
            <p class="text-sm text-brand-gray-light">Privacy policy</p>
        </div>
        <div class="flex-1">
            <h3 class="text-brand-gray-dark-1 font-semibold text-sm mb-4">Know more</h3>
            <p class="mb-4 text-sm text-brand-gray-light">Contact Us</p>
            <p class="text-sm text-brand-gray-light">About Us</p>
        </div>
    </div>
</footer>